...
import yardimcilar.Silahlar_Karakterler.Avengers;

public class ActionBar extends Bar {

	private Oynanis playing;
	...
	public ActionBar(int x, int y, int width, int height, Oynanis playing) {
	...
	private void initButtons() {
		...
		bPause = new MyButton("Durdur", 2, 682, 100, 30);
...
//		sellTower, upgradeTower; 
		sellTower = new MyButton("Sat", 420, 702, 80, 25);
		upgradeTower = new MyButton("Yükselt", 545, 702, 80, 25);
	...
	private void drawButtons(Graphics g) {
		...
		for (MyButton b : towerButtons) {
			...
			g.drawImage(playing.getTowerManager().getAvengers_fotolar()[b.getId()], b.x, b.y, b.width, b.height, null);
	...
	public void draw(Graphics g) {
		...
		if (playing.isGamePaused()) {
			...
			g.drawString("Oyun Durdu!", 110, 790);
		}

		// Lives
		g.setColor(Color.black);
		g.drawString("Canlar: " + lives, 110, 750);
	...
	private String getTowerCostName() {
		return yardimcilar.Silahlar_Karakterler.Avengers.Avengerİsim(towerCostType);
	}

	private int getTowerCostCost() {
		return yardimcilar.Silahlar_Karakterler.Avengers.KarakterMaliyeti(towerCostType);
	}

	private void drawGoldAmount(Graphics g) {
		g.drawString("Lira: " + gold + "₺", 110, 725);
	...
	private void drawWavesLeftInfo(Graphics g) {
		...
		g.drawString("Dalga " + (current + 1) + " / " + size, 425, 770);

	}

	private void drawEnemiesLeftInfo(Graphics g) {
		int remaining = playing.getEnemyManger().kalandusmanlarr();
		g.drawString("Kalan Düşmanlar: " + remaining, 425, 790);
	}
	...
	private void drawWaveTimerInfo(Graphics g) {
		if (playing.getWaveManager().isWaveTimerStarted()) {
			...
			g.drawString("Kalan Zaman: " + formattedText, 425, 750);
		}
	}
	...
	private void drawDisplayedTower(Graphics g) {
		if (displayedTower != null) {
			...
			g.drawImage(playing.getTowerManager().getAvengers_fotolar()[displayedTower.getTowerType()], 420, 650, 50, 50, null);
			g.setFont(new Font("LucidaSans", Font.BOLD, 15));
			g.drawString("" + Avengers.Avengerİsim(displayedTower.getTowerType()), 480, 660);
			g.drawString("ID: " + displayedTower.getId(), 480, 675);
			g.drawString("Level: " + displayedTower.getTier(), 560, 660);
			...
			if (sellTower.isMouseOver()) {
				g.setColor(Color.red);
				g.drawString("Şu Kadara Sat: " + getSellAmount(displayedTower) + "g", 480, 695);
			} else if (upgradeTower.isMouseOver() && gold >= getUpgradeAmount(displayedTower)) {
				g.setColor(Color.blue);
				g.drawString("Şu Kadara Yükselt: " + getUpgradeAmount(displayedTower) + "g", 480, 695);
			}
	...
	private int getUpgradeAmount(Tower displayedTower) {
		return (int) (yardimcilar.Silahlar_Karakterler.Avengers.KarakterMaliyeti(displayedTower.getTowerType()) * 0.3f);
	}
	...
	private int getSellAmount(Tower displayedTower) {
		...
		return yardimcilar.Silahlar_Karakterler.Avengers.KarakterMaliyeti(displayedTower.getTowerType()) / 2 + upgradeCost;
	}
	...
	private void sellTowerClicked() {
		...
		gold += yardimcilar.Silahlar_Karakterler.Avengers.KarakterMaliyeti(displayedTower.getTowerType()) / 2;
	...
	private void togglePause() {
		...
		if (playing.isGamePaused())
			bPause.setText("Başlat");
		else
			bPause.setText("Durdur");
	...
	private boolean isGoldEnoughForTower(int towerType) {
		...
		return gold >= yardimcilar.Silahlar_Karakterler.Avengers.KarakterMaliyeti(towerType);
	}
	...
	public void payForTower(int towerType) {
		this.gold -= yardimcilar.Silahlar_Karakterler.Avengers.KarakterMaliyeti(towerType);
