...
import static yardimcilar.Silahlar_Karakterler.Arkaplanlar.YOL;
...
public class Duzenleme extends GameScene implements SceneMethods {
	...
	public Duzenleme(Machiolations game) {
		super(game);
		loadDefaultLevel();
	...
	public void saveLevel() {
		...
		SpriteAtlasAlma.LevelKayit("new_level", lvl, start, end);
		game.getOynanis().setLevel(lvl);

	}

	public void setSelectedTile(Dokular tile) {
	...
	private void changeTile(int x, int y) {
		if (selectedTile != null) {
			...
			if (selectedTile.getId() >= 0) {
				...
				if (game.getArkaplanayarlma().getFayans(id).getTileType() == YOL) {
					if (selectedTile.getId() == -1)
						start = new PatikaKonumu(tileX, tileY);
					else
						end = new PatikaKonumu(tileX, tileY);
				}
			}
		}
	}
